...
class MKR(object):
    ...
    def _build_inputs(self, args):
        ...
        # self.item_emd = tf.placeholder(tf.float32, [args.batch_size, args.bert_dim], 'item_emd')
        # self.head_emd = tf.placeholder(tf.float32, [args.batch_size, args.bert_dim], 'head_emd')
        # self.tail_emd = tf.placeholder(tf.float32, [args.batch_size, args.bert_dim], 'tail_emd')
        self.item_emd = tf.placeholder(tf.float32, [None, None], 'item_emd')
        self.head_emd = tf.placeholder(tf.float32, [None, None], 'head_emd')
        self.tail_emd = tf.placeholder(tf.float32, [None, None], 'tail_emd')
    ...
    def _build_low_layers(self, args):
        ...
        self.item_emb_matrix = tf.get_variable('item_emb_matrix', [self.n_item, args.bert_dim])
        self.entity_emb_matrix = tf.get_variable('entity_emb_matrix', [self.n_entity, args.bert_dim])
        # self.item_emb_matrix = tf.Variable(np.zeros([args.batch_size, args.bert_dim]), dtype=tf.float32)
        # self.entity_emb_matrix = tf.Variable(np.zeros([args.batch_size, args.bert_dim]), dtype=tf.float32)
        self.relation_emb_matrix = tf.get_variable('relation_emb_matrix', [self.n_relation, args.dim])

        # add
        # self.tail_emb_matrix = tf.Variable(np.zeros([args.batch_size, args.bert_dim]), dtype=tf.float32)
        # self.head_emb_matrix = tf.Variable(np.zeros([args.batch_size, args.bert_dim]), dtype=tf.float32)

        # [batch_size, dim]
        self.user_embeddings = tf.nn.embedding_lookup(self.user_emb_matrix, self.user_indices)
        self.item_embeddings = self.item_emd
        self.head_embeddings = self.head_emd
        self.tail_embeddings = self.tail_emd
        # self.item_embeddings = tf.assign(self.item_emb_matrix, self.item_emd)
        # self.head_embeddings = tf.assign(self.head_emb_matrix, self.head_emd)
        # self.tail_embeddings = tf.assign(self.tail_emb_matrix, self.tail_emd)
        ...
        for _ in range(args.L):
            ...
            tail_mlp = Dense(input_dim=args.bert_dim, output_dim=args.bert_dim)
            cc_unit = CrossCompressUnit(args.bert_dim)
    ...
    def _build_high_layers(self, args):
        ...
        if use_inner_product:
            ...
            for _ in range(args.H - 1):
                rs_mlp = Dense(input_dim=args.dim + args.bert_dim, output_dim=args.dim + args.bert_dim)
                # [batch_size, dim * 2]
                self.user_item_concat = rs_mlp(self.user_item_concat)
                self.vars_rs.extend(rs_mlp.vars)

            rs_pred_mlp = Dense(input_dim=args.dim + args.bert_dim, output_dim=1)
            # [batch_size]
        ...
        for _ in range(args.H - 1):
            kge_mlp = Dense(input_dim=args.dim + args.bert_dim, output_dim=args.dim + args.bert_dim)

            # [batch_size, dim]
            self.head_relation_concat = kge_mlp(self.head_relation_concat)
            self.vars_kge.extend(kge_mlp.vars)

        kge_pred_mlp = Dense(input_dim=args.dim + args.bert_dim, output_dim=args.bert_dim)
        # [batch_size, 1]
